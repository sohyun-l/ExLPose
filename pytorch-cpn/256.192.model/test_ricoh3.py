...
from test_config_ricoh3 import cfg_test
...
def main(args):
    ...
    ExLPoseOC(cfg_test, train=False),
    ...
    for i, (inputs, meta) in tqdm(enumerate(test_loader)):
        with torch.no_grad():
            ...
            # compute output
            t0, t1, t2, t3, t4, global_outputs, refine_output = model(input_var, 0 * torch.ones(input_var.shape[0], dtype=torch.long).cuda())
            score_map = refine_output.data.cpu()
            score_map = score_map.numpy()

            if args.flip == True:
                t0, t1, t2, t3, t4, flip_global_outputs, flip_output = model(flip_input_var, 0 * torch.ones(flip_input_var.shape[0], dtype=torch.long).cuda())
    ...
    result_path = 'Qual_generalization_Ours'
